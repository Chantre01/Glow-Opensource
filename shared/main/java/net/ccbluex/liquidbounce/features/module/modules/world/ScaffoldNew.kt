...
class ScaffoldNew : Module() {
    ...
    // Basic stuff
    @JvmField
    public val sprintValue = BoolValue("Sprint", false)
    val sprintOnGround = BoolValue("SprintOnGround", false)
    private val swingValue = BoolValue("Swing", true)
    private val searchValue = BoolValue("Search", true)
    private val downValue = BoolValue("Down", true)
    private val placeModeValue = ListValue("PlaceTiming", arrayOf("Pre","Post","Tick","PreAndPost"), "Post")
    ...
    //Reach
    private val range = FloatValue("Range",4.25f,0f,6.25f)
    ...
    //Telly (ShitCode)
    @JvmField
    val tellyMode = BoolValue("TellyMode", true)
    private val airPlaceDelay = IntegerValue("AirPlaceDelay",0,0,1000)
    private val airRotDelay = IntegerValue("AirRotDelay",0,0,1000)
    private val airRotTimer = MSTimer()
    private val airPlaceTimer = MSTimer()
    ...
    private fun onTick(event: TickEvent){
        ...
        if((mc.thePlayer!!.fallDistance > 0&&canPlace)){
            place()
        }
    }
    ...
    private fun onUpdate(event: UpdateEvent) {
        ...
        if (player.onGround) {
            ...
            if(airRotTimer.hasTimePassed(airRotDelay.get().toLong())){
                canRot = true
            }
            if(airPlaceTimer.hasTimePassed(airPlaceDelay.get().toLong())){
                canPlace = true
            }
        }
    }
    ...
    fun onMotion(event: MotionEvent) {
        ...
        // Face block
        if ((facesBlock || !rotationsValue.get()) && (placeModeValue.get().equals(eventState.stateName, true) || placeModeValue.get().equals("PreAndPost", true))) {
            place()
        }
            ...
            while (xSearch <= if (auto) 0.9 else 0.5 + xzRV / 2) {
                ...
                while (ySearch <= if (auto) 0.9 else 0.5 + yRV / 2) {
                    ...
                    while (zSearch <= if (auto) 0.9 else 0.5 + xzRV / 2) {
                        ...
                        if (raycast && (eyesPos.distanceTo(hitVec) > range.get() || distanceSqPosVec > eyesPos.squareDistanceTo(
                                posVec.add(dirVec)
