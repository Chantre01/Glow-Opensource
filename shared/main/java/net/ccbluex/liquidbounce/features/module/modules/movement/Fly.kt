...
class Fly : Module() {
    ...
    private val grimxValue = FloatValue("Grim-XValue", 1000f, 1f, 3000f)
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "flag" -> {
                    ...
                    thePlayer.setPosition(
                        thePlayer.posX + thePlayer.motionX * 11,
                        thePlayer.posY,
                        thePlayer.posZ + thePlayer.motionZ * 11
                    )
                    thePlayer.motionY = 0.0
                }
...
//                    mc.effectRenderer.spawnEffectParticle(EnumParticleTypes.FLAME.getParticleID(), thePlayer.posX, thePlayer.posY + 0.2, thePlayer.posZ, -thePlayer.motionX, -0.5, -thePlayer.motionZ)
    ...
    fun onMotion(event: MotionEvent) {
        if (modeValue.get().equals("grimdamage", ignoreCase = true)) {
            if (event.eventState == EventState.PRE) {
                mc.thePlayer!!.setPositionAndRotation(
                    mc.thePlayer!!.posX + grimxValue.get(),
                    mc.thePlayer!!.posY,
                    mc.thePlayer!!.posZ,
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (classProvider.isCPacketPlayer(event.packet)) {
            ...
            if (mode.equals("NCP", ignoreCase = true) || mode.equals("Rewinside", ignoreCase = true) ||
                mode.equals("Mineplex", ignoreCase = true) && mc.thePlayer!!.inventory.getCurrentItemInHand() == null
    ...
    fun onMove(event: MoveEvent) {
        when (modeValue.get().toLowerCase()) {
            ...
            "boosthypixel" -> {
                ...
                val amplifier =
                    1 + (if (mc.thePlayer!!.isPotionActive(classProvider.getPotionEnum(PotionType.MOVE_SPEED))) 0.2 *
