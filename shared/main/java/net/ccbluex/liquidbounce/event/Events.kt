...
class EntityKilledEvent(val targetEntity: IEntity?) : Event()
...
class MoveInputEvent(val forward: Float = 0f, val strafe: Float = 0f, val jump: Boolean = false, val sneak: Boolean = false, sneakSlowDownMultiplier: Double = 0.0) : Event() {
    fun setForward(forward: Float) {
        //setForward(forward)
        MovementUtils.forward(forward.toDouble())
    }
    fun setStrafe(strafe: Float) {
        //setStrafe(strafe)
...
class ClickUpdateEvent() : CancellableEvent()