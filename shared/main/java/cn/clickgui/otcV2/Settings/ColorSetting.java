...
import static cn.utils.miku.render.RenderUtil.darker;
...
public class ColorSetting extends Downward<ColorValue> {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        //Value名字
        Fonts.SFBOLD.SFBOLD_11.SFBOLD_11.drawString(colorValue.getName(), modulex + 5 + pos.x + 4, moduley + 17 + colory + 12, new Color(200, 200, 200).getRGB());
        ...
        VisualBase.drawGradientRect(x2, y2, x2 + width, y2 + height, newColor, darker(newColor, 0.6f));
        if (colorValue.isExpanded()) {
            ...
            while ((float)i2 < 5.0f) {
                ...
                VisualBase.drawGradientRect(hueSliderLeft, asLeft, hueSliderRight, asLeft + alphaSelectorX, RenderUtil.getColor(Color.HSBtoRGB(1.0f - 0.2f * (float)i2, 1.0f, 1.0f)), RenderUtil.getColor(Color.HSBtoRGB(1.0f - 0.2f * (float)(i2 + 1), 1.0f, 1.0f)));
                if (!last) {
