...
object ShadowUtils : MinecraftInstance() {
    ...
    fun initShaderIfRequired(sc: ScaledResolution, strength: Float) {
        ...
        if (lastWidth != width || lastHeight != height
            ...
            shaderGroup = ShaderGroup(mc2.textureManager, mc2.getResourceManager(), initFramebuffer, blurDirectory)
    ...
    fun shadow(strength: Float, drawMethod: (() -> Unit), cutMethod: (() -> Unit)) {
        ...
        mc2.getFramebuffer().unbindFramebuffer()
        ...
        mc2.getFramebuffer().bindFramebuffer(true)
        ...
        worldrenderer.begin(7, classProvider.getVertexFormatEnum(WDefaultVertexFormats.POSITION_TEX_COLOR))
        worldrenderer.pos(0.0, height.toDouble(), 0.0).tex(0.0, 0.0).color(255f, 255f, 255f, 255f).endVertex()
        worldrenderer.pos(width.toDouble(), height.toDouble(), 0.0).tex(fr_width, 0.0).color(255f, 255f, 255f, 255f).endVertex()
        worldrenderer.pos(width.toDouble(), 0.0, 0.0).tex(fr_width, fr_height).color(255f, 255f, 255f, 255f).endVertex()
        worldrenderer.pos(0.0, 0.0, 0.0).tex(0.0, fr_height).color(255f, 255f, 255f, 255f).endVertex()
