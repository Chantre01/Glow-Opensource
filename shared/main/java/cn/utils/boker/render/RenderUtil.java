...
import cn.clickgui.skeet.Animation;
...
//        Helper.mc.entityRenderer.setupCameraTransform(Helper.mc.timer.renderPartialTicks, 0);
    ...
    public static void drawArc(float n, float n2, double n3, final int n4, final int n5, final double n6, final float n7) {
        n3 *= 2.0;
        n *= 2.0f;
        n2 *= 2.0f;
    ...
    public static void drawScaledCustomSizeModalCircle(int x, int y, float u, float v, int uWidth, int vHeight, int width, int height, float tileWidth, float tileHeight) {
        ...
        ITessellator tessellator = classProvider.getTessellatorInstance();
        IWorldRenderer bufferbuilder = tessellator.getWorldRenderer();
        bufferbuilder.begin(7, classProvider.getVertexFormatEnum(WDefaultVertexFormats.POSITION_TEX));
