...
public class DirtyFontRenderer {
    ...
    public DirtyFontRenderer(Font font) {
        ...
        for (char c : CHARS.toCharArray()) {
            ...
            BufferedImage bufferedImage = new BufferedImage(MathHelper.ceil(cWidth), MathHelper.ceil(cHeight), BufferedImage.TYPE_INT_ARGB); // round up if the char is a point number that chars are not cut off.
    ...
    public void drawString(String str, float x, float y, Color color, boolean dropShadow) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (c == '§' && i + 1 < chars.length) {
                int mcColor = Minecraft.getMinecraft().fontRenderer.getColorCode(chars[i + 1]);
                ...
                ClientUtils.getLogger().warn("Unknown letter: " + c);
            }
        }
