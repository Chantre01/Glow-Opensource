        ...
        fun isBot(entity: IEntityLivingBase): Boolean {
            ...
            if (antiBot.tabValue.get()) {
                ...
                if (targetName != null) {
                    for (networkPlayerInfo in mc.netHandler.playerInfoMap) {
                        val networkName = stripColor(EntityUtils2.getName(networkPlayerInfo)) ?: continue
            ...
            if (antiBot.duplicateInTabValue.get()) {
                if (mc.netHandler.playerInfoMap.stream()
                        .filter { networkPlayer: INetworkPlayerInfo? -> entity.name == stripColor(EntityUtils2.getName(networkPlayer)) }
                        .count() > 1) return true
            }
