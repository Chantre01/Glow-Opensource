...
@ModuleInfo(name = "ScaffoldII", description = "Sk1d by 2997570499.", category = ModuleCategory.WORLD, keyBind = Keyboard.KEY_I)
public class ScaffoldII extends Module {
    ...
    private final BoolValue AirRotValue = new BoolValue("AirRotation", false);

    private final FloatValue AirRotSetValue = new FloatValue("AirRotationDelay", 0F, 0F, 1F);

    private final ListValue rotationStrafeValue = new ListValue("RotationStrafe", new String[]{"Off", "Silent", "test"}, "Off");
    ...
    @EventTarget
    //took it from applyrotationstrafe XD. staticyaw comes from bestnub.But bucause bugs so now changed it sk1d from Liquidbounce scaffold
    public void onStrafe(StrafeEvent event) {
        if (rotationStrafeValue.get().equalsIgnoreCase("Off")) {
            return;
        }

        switch (rotationStrafeValue.get().toLowerCase()){
            //skid killaura strict
            case "silent":
                RotationUtils.serverRotation.applyStrafeToPlayer(event);
                event.cancelEvent();
                break;
            case "test":
                float yaw = lookupRotation.getYaw();
                ...
                if (f >= 1.0E-4F) {
                    ...
                    strafe *= f;
                    forward *= f;
                    float yawCos = (float)((double)yaw * Math.PI / (double)180.0F);
                    float yawSin = (float)Math.sin((double)yawCos);
                    mc.getThePlayer().setMotionX(strafe * yawCos - forward * yawSin);
                    mc.getThePlayer().setMotionZ(forward * yawCos + strafe * yawSin);
                }
                event.cancelEvent();
            }
