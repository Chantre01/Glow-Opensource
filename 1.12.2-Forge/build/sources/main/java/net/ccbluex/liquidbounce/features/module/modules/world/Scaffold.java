...
@ModuleInfo(
        name = "Scaffold",
        description = "AutoPlace Blocks Under your feet",
...
public class Scaffold extends Module {
    ...
    public void onRender2D(final Render2DEvent event) {
        if (counterDisplayValue.get()) {
            ...
            RenderUtils.drawRoundedRect2(scaledResolution.getScaledWidth() / 2 - (infoWidth / 2), scaledResolution.getScaledHeight() - 74, 8+Fonts.font35.getStringWidth(info), 14, 2F, new Color(0.15F, 0.15F, 0.15F, progress).getRGB());
            ...
            if(hotbarContainBlock()){
                for (int a = 36; a < 45; a++) {
                    if (mc.getThePlayer().getInventoryContainer().getSlot(a).getHasStack()) {
                        IItemStack item = mc.getThePlayer().getInventoryContainer().getSlot(a).getStack();
            ...
            if (invCheck()) {
                ...
                for (int i = 9; i < 36; i++) {
                    ...
                    if (mc.getThePlayer().getInventoryContainer().getSlot(i).getHasStack()) {
                        IItemStack item = mc.getThePlayer().getInventoryContainer().getSlot(i).getStack();
                        int count = 0;
