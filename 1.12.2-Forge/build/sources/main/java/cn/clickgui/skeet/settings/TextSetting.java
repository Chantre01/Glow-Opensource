...
public class TextSetting extends Downward<TextValue> {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        FontLoaders.NL18.drawString(setting.getName(),mainx + 17 + pos.x,mainy + 38 + y,new Color(130,140,150).getRGB());

        RoundedUtil.drawRound(mainx + 91 + pos.x, mainy + 35 + y, 70, 12, 0,new Color(5,23,37));

        //��ʾ�������� �жϳ���
        if (FontLoaders.NL16.getStringWidth(setting.getTextHovered() ? setting.get() + "_": setting.get()) > 65) {
            //�޼��ַ�������
            FontLoaders.NL16
                    .drawString(FontLoaders.NL16.trimStringToWidth(setting.getTextHovered() ? setting.get() + "_": setting.get(), 59, true),mainx + 93 + pos.x,mainy + 38 + y,
                            new Color(130,140,150).getRGB());
        } else {
            if (setting.get().isEmpty() && !setting.getTextHovered()){
                FontLoaders.NL16.drawString("Type Here...",mainx + 93 + pos.x,mainy + 38 + y, new Color(130,140,150).getRGB());
            }else {
                FontLoaders.NL16.drawString(setting.getTextHovered() ? setting.get() + "_" : setting.get(),mainx + 93 + pos.x,mainy + 38 + y, new Color(130,140,150).getRGB());
            }
        }
    }
