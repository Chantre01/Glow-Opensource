...
public class GuiInputBox extends Gui {
    ...
    public GuiInputBox(int componentId, CFontRenderer fontrendererObj, int x, int y, int par5Width, int par6Height) {
        this.id = componentId;
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            String s = Minecraft.getMinecraft().fontRenderer.trimStringToWidth((text.isEmpty() ? placeholder : text).substring(this.lineScrollOffset), this.getWidth());
            ...
            if (s.length() > 0) {
                ...
                FontLoaders.jelloFontMedium16.drawString(s1, l, i1, i);
                j1 += FontLoaders.jelloFontMedium16.getStringWidth(s1);
            }
            ...
            if (s.length() > 0 && flag && j < s.length()) {
                FontLoaders.jelloFontMedium16.drawString(s.substring(j), j1, i1, i);
            }

            if (flag1) {
                if (flag2) {
                    Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + FontLoaders.jelloFontMedium16.getHeight(), -3092272);
                } else {
                    FontLoaders.jelloFontMedium16.drawString("_", k1, i1, i);
                }
            }

            if (k != j) {
                int l1 = l + this.fontRendererInstance.getStringWidth(s.substring(0, k));
                this.drawCursorVertical(k1, i1 - 1, l1 - 1, i1 + 1 + FontLoaders.jelloFontMedium16.getHeight());
            }
        }
    }
