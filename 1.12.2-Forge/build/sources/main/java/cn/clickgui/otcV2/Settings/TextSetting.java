...
public class TextSetting extends Downward<TextValue> {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        //框
        RenderUtil.drawRoundedRect(modulex+ 5+ pos.x + 55 ,moduley+ 17 +texty + 8,75,11f,1, new Color(59,63,72).getRGB(),1, textValue.getTextHovered() ?new Color(UIEffects.Hudcolor.get()).getRGB():new Color(85,90,96).getRGB());

        //显示描边
        if (isHovered(mouseX,mouseY)&& !textValue.getTextHovered()){
            RenderUtil.drawRoundedRect(modulex+ 5+ pos.x + 55 ,moduley+ 17 +texty + 8,75,11f,1, new Color(0,0,0,0).getRGB(),1,new Color(UIEffects.Hudcolor.get()).getRGB());
        }
        //文字
        Fonts.SFBOLD.SFBOLD_11.SFBOLD_11.drawString(textValue.getName(),modulex+ 5+ pos.x + 4 ,moduley+ 17 +texty + 13,new Color(200,200,200).getRGB());

        //显示框内文字 判断长度
        if (Fonts.SFBOLD.SFBOLD_12.SFBOLD_12.stringWidth(textValue.getTextHovered() ? textValue.get() + "_": textValue.get()) > 70) {
            //修剪字符串长度
            Fonts.SFBOLD.SFBOLD_12.SFBOLD_12
                    .drawString(Fonts.SFTHIN.SFTHIN_16.SFTHIN_16.trimStringToWidth(textValue.getTextHovered() ? textValue.get() + "_": textValue.get(), 78, true),modulex+ 5+ pos.x + 57 ,moduley+ 17 +texty + 13,
                            new Color(200,200,200).getRGB());
        } else {
            if (textValue.get().isEmpty() && !textValue.getTextHovered()){
                Fonts.SFBOLD.SFBOLD_12.SFBOLD_12.drawString("Type Here...",modulex+ 5+ pos.x + 57 ,moduley+ 17 +texty + 13,new Color(200,200,200).getRGB());
            }else {
                Fonts.SFBOLD.SFBOLD_12.SFBOLD_12.drawString(textValue.getTextHovered() ? textValue.get() + "_" : textValue.get(), modulex + 5 + pos.x + 57, moduley + 17 + texty + 13, new Color(200, 200, 200).getRGB());
            }
        }
    }

    public boolean isHovered(int mouseX, int mouseY) {
        return mouseX >=modulex+ 5+ pos.x + 55 && mouseX <= modulex+ 5+ pos.x + 55 + 75 && mouseY >= moduley+ 17 +texty + 8 && mouseY <= moduley+ 17 +texty + 8 + 11f;
    }
    ...
    public void keyTyped(char typedChar,int keyCode) {
            ...
            //删除
            if (setting.getTextHovered() && Keyboard.isKeyDown(Keyboard.KEY_BACK) && textValue.get().length() >= 1) {
                textValue.set(textValue.get().substring(0, textValue.get().length() - 1));
              }
         }
    }
